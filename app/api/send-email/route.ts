...
export async function GET(request: NextRequest) {
    try {
        ...
        // Fetch all event UUIDs that have already occurred at least 4 hours ago and where rated=false
        const pastEvents = await db.select({
            uuid: events.uuid,
            eventName: events.eventName
        })
            .from(events)
            .where(lt(events.dateTime, currentTime.toISOString()))
            .execute();

        const eventUuids = pastEvents.map(event => event.uuid);

        if (eventUuids.length === 0) {
            console.log("No past events found or all events have been rated.");
            return new Response(JSON.stringify({ success: true }), {
                status: 200
            });
        }

        // Fetch customers related to these events who haven't rated them yet and haven't been sent an email
        //@ts-ignore
        const conditions = eventUuids.map(uuid => eq(eventCustomers.eventUuid, uuid));
        //@ts-ignore
        const combinedCondition = conditions.reduce((acc, condition) => or(acc, condition));

        const customerEvents = await db.select({
            id: eventCustomers.id,
            ...
            .from(eventCustomers)
            .where(and(eq(eventCustomers.rated, false), combinedCondition, eq(eventCustomers.sentEmail, false)))
            .orderBy(desc(eventCustomers.id))
            .limit(200) // Limit to first 100 customers
            .execute();
        ...
        // Matching event names with customer names for the console log and sending emails
        for (const customerEvent of customerEvents) {
            const eventName = pastEvents.find(event => event.uuid === customerEvent.eventUuid)?.eventName;
            console.log(`Hello, ${customerEvent.firstName}, please rate the event: ${eventName}`);

            let info = await transporter.sendMail({
                from: '"Eventify" ' + process.env.EMAIL_FROM,
                to: customerEvent.email,
                subject: "Вашето мнение за "+eventName,
                ...
                <td align="center" style="padding:0;Margin:0;padding-bottom:20px;font-size:0px"><a target="_blank" href="https://www.eventify.bg/" style="-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;text-decoration:underline;color:#666666;font-size:14px"><img src="https://www.eventify.bg/logo.png" alt="Logo" style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" width="200" title="Logo"></a> </td></tr></table></td></tr></table></td></tr></table></td></tr></table> <table cellpadding="0" cellspacing="0" class="es-content" align="center" role="none" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;table-layout:fixed !important;width:100%"><tr>\
                ...
                <td align="center" class="es-m-txt-c" style="padding:0;Margin:0;padding-bottom:10px"><h1 style="Margin:0;line-height:46px;mso-line-height-rule:exactly;font-family:arial, \'helvetica neue\', helvetica, sans-serif;font-size:46px;font-style:normal;font-weight:bold;color:#333333">Вашето мнение за '+eventName+'</h1> </td></tr><tr><td align="center" style="padding:0;Margin:0;padding-top:10px;padding-bottom:10px;font-size:0px"><img class="adapt-img" src="https://ecgujso.stripocdn.email/content/guids/CABINET_b297a3f42cdd825f6211aa1ebcaf0723/images/82701618311893051.png" alt style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" width="260"></td></tr> <tr>\
                <td align="center" class="es-m-p0r es-m-p0l" style="Margin:0;padding-top:5px;padding-bottom:5px;padding-left:40px;padding-right:40px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, \'helvetica neue\', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Здравейте '+customerEvent.firstName+' '+customerEvent.lastName+'! Благодарим Ви, че участвахте в '+eventName+'. Ние в Eventify се стремим да подобрим опита на нашите потребители и Вашето мнение е изключително важно за нас.</p>\
                 <p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, \'helvetica neue\', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Бихме били изключително благодарни, ако отделите няколко минути, за да попълните кратка анкета, чрез която да споделите Вашите впечатления и предложения за подобрения. Вашето мнение е ценно за нас!</p><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, \'helvetica neue\', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Моля, натиснете на бутона по-долу, за да достъпите анкетата. Тя ще Ви отнеме само няколко минути.</p>\
                 <p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, \'helvetica neue\', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px">Благодарим Ви предварително за времето и съдействието.</p></td></tr> <tr>\
                <td align="center" style="padding:0;Margin:0;padding-top:20px;padding-bottom:20px"><span class="es-button-border" style="border-style:solid;border-color:#5c68e2;background:#5c68e2;border-width:2px;display:inline-block;border-radius:6px;width:auto"><a href="https://www.eventify.bg/rate/'+customerEvent.ticketToken+'" class="es-button es-button-1621629131049" target="_blank" style="mso-style-priority:100 !important;text-decoration:none;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;color:#FFFFFF;font-size:20px;padding:10px 30px;display:inline-block;background:#5C68E2;border-radius:6px;font-family:arial, \'helvetica neue\', helvetica, sans-serif;font-weight:normal;font-style:normal;line-height:24px;width:auto;text-align:center;mso-padding-alt:0;mso-border-alt:10px solid #5C68E2"> <!--[if !mso]><!-- --><img src="https://ecgujso.stripocdn.email/content/guids/CABINET_b297a3f42cdd825f6211aa1ebcaf0723/images/50181618310504851.png" alt="icon" width="20" style="display:inline-block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic;vertical-align:middle;margin-right:10px" align="absmiddle"> <!--<![endif]-->ПОПЪЛНИ АНКЕТА </a>\
                 </span></td></tr></table></td></tr></table></td></tr></table></td></tr></table> <table cellpadding="0" cellspacing="0" class="es-content" align="center" role="none" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;table-layout:fixed !important;width:100%"><tr><td align="center" style="padding:0;Margin:0"><table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" role="none" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;background-color:#FFFFFF;width:600px"><tr><td align="left" style="padding:0;Margin:0;padding-top:20px;padding-left:20px;padding-right:20px"><table cellpadding="0" cellspacing="0" width="100%" role="none" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"><tr>\
        ...
        // Update sentEmail to true for all fetched customerEvents
        const customerEventIds = customerEvents.map(customerEvent => customerEvent.id);
        if (customerEventIds.length > 0) {
            await db.update(eventCustomers)
                .set({ sentEmail: true })
                .where(inArray(eventCustomers.id, customerEventIds))
                .execute();
        }
